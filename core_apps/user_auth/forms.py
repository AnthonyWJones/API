...
class UserCreationForm(DjangoUserCreationForm):
    ...
    def clean_id_no(self):
        ...
        if User.objects.filter(id_no=id_no).exists():
            raise ValidationError(_("This ID number is already registered. Please use a different one."))
        return id_no
...
class UserChangeForm(DjangoUserChangeForm):
    class Meta:
        ...
        fields = [
            "email", "id_no", "first_name", "middle_name", "last_name",
            "security_question", "security_answer", "is_staff", "is_superuser", "is_active"
        ]
